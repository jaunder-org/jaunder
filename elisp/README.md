...
Media Copies** are durable blog content, not a cache: include `local-media/` in
backups and do not expect automatic eviction or repair. The configured root is
trusted, author-owned local state; the client rejects symlinks during creation
and immediately before mutation, but cannot defend a malicious replacement after
its final check without dirfd APIs.
