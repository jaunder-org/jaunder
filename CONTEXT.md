...
**Rendered Title**: A Post title's persisted, sanitized, inline-only HTML
projection in the Post's authoring format. Web headings and markup-capable
Syndication Feed fields use it directly; plain-text feed fields derive visible
text from it. The authored Post title remains canonical for editing, slugs,
metadata, and AtomPub. _Avoid_: formatted title (does not name the persisted
projection), rendered heading (the heading also contains presentation-owned
structure).
