...
/// [`ContentHash`], [`MediaSource`], and [`Filename::from_decoded_segment`] — which is
/// where the ADR's single-definition point actually applies.
///
/// The filename segment is percent-decoded and re-encoded through
/// [`Filename::from_decoded_segment`], so a post addressing the file by its raw name and
...
pub fn parse_media_url(url: &str) -> Option<MediaRef> {
    ...
    // arbiter of whether the name *could* be stored at all — an unsafe leaf or an
    // over-budget name names nothing, so it is not a reference.
