...
/// misconfiguration. The paired secret is the host-only `SmtpPassword`; making
/// both typed keeps the credential half of `SmtpConfig` a fully-typed pair (no
/// same-typed transposition at the lettre `Credentials` boundary). No trim —
/// the stored value is used verbatim. Web-settable wiring (a typed wire arg,
/// non-secret) is #638.
