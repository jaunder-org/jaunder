...
///
/// The trailer (`Display`, `AsRef`/`Borrow`/`Deref<str>`, owned-`String`
/// conversions, `PartialEq<str>`/`PartialEq<&str>`, and the validating serde
/// bridge) is generated by `#[derive(StrNewtype)]` (ADR-0063); only the
/// normalizing `FromStr` below is hand-written. No `Hash` — nothing hashes a
/// `Slug`; ordering is emitted by the trailer (ADR-0063 §2).
...
mod tests {
    ...
    fn slug_accepts_ascii_and_unicode_lowercasing() {
        ...
        assert_eq!("café".parse::<Slug>().unwrap(), "café");
    }
    ...
    fn slug_preserves_indic_conjunct_marks() {
        ...
        for word in ["नमस्ते", "हिन्दी", "தமிழ்"] {
            ...
            assert_eq!(slugify_title(word).parse::<Slug>().unwrap(), nfc.as_str());
        }
    }
    ...
    fn slug_keeps_alphabetic_marks_regression() {
        ...
        assert_eq!(arabic.parse::<Slug>().unwrap(), nfc.as_str());
    }
    ...
    fn slug_normalizes_nfd_input_to_nfc() {
        ...
        let nfd = "cafe\u{0301}";
        assert_eq!(nfd.parse::<Slug>().unwrap(), "café");
        assert_eq!(
    ...
    fn slug_display_returns_inner_string() {
        ...
        assert_eq!(s, "my-post"); // generated PartialEq<&str>
    }
