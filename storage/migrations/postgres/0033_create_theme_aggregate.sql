...
CREATE TABLE theme_drafts (
    theme_id BIGINT PRIMARY KEY
        REFERENCES themes(id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED,
    manifest BYTEA NOT NULL,
    stylesheet BYTEA NOT NULL,
...
ALTER TABLE themes
    ADD CONSTRAINT themes_current_revision_belongs_to_theme
...
CREATE TABLE theme_content_eligibility (
    ...
    live_references BIGINT NOT NULL DEFAULT 0 CHECK (live_references >= 0)
);
...
CREATE TABLE theme_role_bindings (
    ...
    CHECK (
        ...
        OR (mode = 'pool'
            ...
            AND octet_length(shuffle_seed) = 32)
    )
);
...
CREATE TABLE theme_owner_quotas (
    catalog_owner_key TEXT PRIMARY KEY,
    active_themes BIGINT NOT NULL DEFAULT 0 CHECK (active_themes >= 0),
    retained_revisions BIGINT NOT NULL DEFAULT 0 CHECK (retained_revisions >= 0),
