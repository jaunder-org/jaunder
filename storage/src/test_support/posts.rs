...
pub struct SeededPost {
    ...
    pub rendered_title: common::render::RenderedPostTitle,
}
...
///
/// There is deliberately no `.idempotency_key` or `.rendered_html` setter: no
/// adopting site supplies an idempotency key or rendered HTML — the builder derives both
/// rendered fields from the authored source with the production renderer.
...
impl SeedRawPost {
    ...
    /// Override the authored title. The Rendered Title re-derives with the body.
    #[must_use]
    ...
    /// without writing. Org fixtures instead use `.seed`/`.create` to reserve
    /// an identity before rendering.
    ///
    /// # Panics
    /// Panics if called for Org without a reserved identity.
    #[must_use]
    pub fn build(self) -> CreatePostInput {
        self.into_input(None)
    }

    fn into_input(self, reserved_post_id: Option<PostId>) -> CreatePostInput {
        ...
        assert!(
            self.format != PostFormat::Org || reserved_post_id.is_some(),
            "Org fixtures require an identity before rendering"
        );
        let rendered = if let Some(post_id) = reserved_post_id {
            host::render::render_post_scoped(post_id, Some(title), self.body, self.format)
        } else {
            host::render::render_post(Some(title), self.body, self.format)
        }
    ...
    /// # Errors
    ///
    /// Propagates ID reservation or [`CreatePostError`] from `create_post`.
    ///
    /// # Panics
    ///
    ...
    pub async fn create(
        ...
        let reserved_post_id = if self.format == PostFormat::Org {
            Some(crate::reserve_post_id(&write_scope, Arc::clone(&posts)).await?)
        } else {
            None
        };
        let input = self.into_input(reserved_post_id);
...
impl UpdateRawPost {
    ...
    /// Remove the authored title and its Rendered Title.
    #[must_use]
    pub fn titleless(mut self) -> Self {
        self.title = None;
        self
    }
    ...
    /// Resolve a non-Org fixture into an [`UpdatePostInput`] without an identity.
    /// Org fixtures must call [`Self::build_for_post`] instead.
    ///
    /// # Panics
    /// Panics for Org or if a fixture's Post body fails to render.
    #[must_use]
    pub fn build(self) -> UpdatePostInput {
        assert_ne!(
            self.format,
            PostFormat::Org,
            "Org requires the target Post ID"
        );
        self.build_with_identity(None)
    }

    /// Render an update using the existing Post's stable ID.
    ///
    /// # Panics
    /// Panics if a fixture's Post body fails to render.
    #[must_use]
    pub fn build_for_post(self, post_id: PostId) -> UpdatePostInput {
        self.build_with_identity(Some(post_id))
    }

    fn build_with_identity(self, post_id: Option<PostId>) -> UpdatePostInput {
        let rendered = if let Some(post_id) = post_id {
            host::render::render_post_scoped(post_id, self.title, self.body, self.format)
        } else {
            host::render::render_post(self.title, self.body, self.format)
        }
        .expect("fixture Post rendering must succeed");
        UpdatePostInput {
            slug: self.slug,
...
#[cfg(test)]
mod tests {
    use super::{SeedPost, SeedRawPost, UpdateRawPost, confirmed_for};
    ...
    #[apply(backends)]
    #[tokio::test]
    async fn org_fixture_create_and_update_keep_post_scoped_footnotes(#[case] backend: Backend) {
        ...
        let post = SeedRawPost::new(author)
            .format(PostFormat::Org)
            .body(parse_post_body("First[fn:one]\n\n[fn:one] Original note."))
            .seed(env.posts(), env.write_scope())
            .await;
        let id = post.post_id;
        let target = format!("id=\"post-{id}-fn-1\"");
        assert!(post.rendered_html.contains(&target));
        let input = UpdateRawPost::new(post.slug.as_ref())
            .format(PostFormat::Org)
            .body(parse_post_body("Again[fn:one]\n\n[fn:one] Revised note."))
            .build_for_post(id);
        let posts = env.posts();
        let outcome = env
            .write_scope()
            .run(move |tx| Box::pin(async move { posts.update_post(tx, id, author, &input).await }))
            .await
            .unwrap();
        confirmed_for(outcome, "Org fixture update");
        let record = env
            .posts()
            .get_post_by_id(id, &ViewerIdentity::local(author))
            .await
            .unwrap()
            .unwrap();
        assert!(record.rendered_html.contains(&target));
        assert!(record.rendered_html.contains("Revised note"));
    }
