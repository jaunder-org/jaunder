...
/// Assembles the hybrid-window SQL for `surface` using the backend-owned tag
/// aggregation and a visibility predicate generated by [`visibility::resolution_where`].
///
/// Pure string construction with no query execution: the four near-identical
...
/// generic `where`-clause, per-surface bind list, and execution. Both dynamic
/// structural fragments have fixed provenance: tags come from
/// [`PostDialect::TAGS_SUBQUERY`] and `resolution` is opaque outside its owner.
fn window_sql<DB: PostDialect>(
    surface: &common::feed::FeedSurface,
...
mod tests {
    ...
    #[test]
    fn publication_window_sql_preserves_surface_specific_bind_order_and_owned_fragments() {
        ...
        let (resolution, _, _) = visibility::resolution_where(&ViewerIdentity::Anonymous, 6);

        for (surface, source_bind, window_binds) in cases {
            let sql = window_sql::<sqlx::Sqlite>(&surface, &resolution);
            assert!(
            ...
            assert!(
                ...
                "{surface:?}: backend-owned tag projection"
            );
            assert!(
                sql.0.contains("AND ( p.user_id = $6"),
                "{surface:?}: generated resolution predicate"
            );
        }
    }
}
