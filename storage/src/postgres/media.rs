...
use crate::posts::store::{self, MediaReferenceEvidence};
...
impl MediaDialect for Postgres {
    ...
    async fn lock_media_reference(
        ...
        let key = store::media_advisory_lock_key(media);
    ...
    async fn try_delete_media(
        ...
        store::push_other_owner_media_reference_from_where(&mut query, user_id, media);
        store::push_live_media_reference_predicate(&mut query, current_instance_id);
    ...
    async fn media_entry_is_reclaimable(
        ...
        query
            ...
            .push_storage_bind(media.filename.clone());
        query.push(") AND NOT EXISTS (SELECT 1");
        store::push_any_media_reference_from_where(&mut query, media);
        store::push_live_media_reference_predicate(&mut query, current_instance_id);
        query.push(")");
        Ok(query
            .build_query_scalar::<i32>()
