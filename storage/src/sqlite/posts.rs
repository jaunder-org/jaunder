...
impl PostStorage for SqlitePostStorage {
    ...
    async fn list_published_in_window(
        ...
        // We use ROW_NUMBER() to identify the top `min_items` posts, then
        // OR-combine with `published_at >= cutoff` to produce the hybrid-window
        // union in a single query.
        let cutoff = window.cutoff_date(now);
        let min_items = i64::from(window.min_items);
        let rows =
            list_published_in_window_sqlite(&self.pool, surface, now, cutoff, min_items).await?;
        rows.into_iter().map(post_record_from_row).collect()
    }
}

const TAGS_SUBQUERY: &str =
    "COALESCE((SELECT json_group_array(json_object('tag_id', t.tag_id, 'tag_slug', t.tag_slug, 'tag_display', pt.tag_display)) FROM post_tags pt JOIN tags t ON pt.tag_id = t.tag_id WHERE pt.post_id = p.post_id), '[]')";

async fn list_published_in_window_sqlite(
    ...
    match surface {
        FeedSurface::Site => window_site_sqlite(pool, now, cutoff, min_items).await,
        FeedSurface::User { username } => {
            window_user_sqlite(pool, username, now, cutoff, min_items).await
        }
        FeedSurface::SiteTag { tag } => {
            window_site_tag_sqlite(pool, tag, now, cutoff, min_items).await
        }
        FeedSurface::UserTag { username, tag } => {
            window_user_tag_sqlite(pool, username, tag, now, cutoff, min_items).await
        }
    }
}

async fn window_site_sqlite(
    pool: &sqlx::SqlitePool,
    now: DateTime<Utc>,
    cutoff: DateTime<Utc>,
    min_items: i64,
