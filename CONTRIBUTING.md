...
Jaunder uses OpenTelemetry for deep performance analysis (see [ADR-0011](docs/adr/0011-unified-observability.md)).

- **No PII in telemetry**: span fields and the structured error boundary (`error.source`/`error.context`) must never carry user PII or secrets (emails, tokens, passwords, post bodies); use stable identifiers like `user_id`, `db.system`, and `error.kind` instead. See [ADR-0011](docs/adr/0011-unified-observability.md).
...
Classification is keyed on **uncovered-line text**, not line number: each accepted-uncovered gap is mapped through the diff, but the mapped line is accepted only when its source text confirms it; otherwise the gap is found by its text among the current lines (ADR-0032, refining ADR-0030). So a line-shifting change — your own edit, a concurrently-merged branch, **or an upstream shift pulled in by the pre-PR rebase that the anchor diff cannot see** — does not desync the baseline: `cargo xtask validate` finds the moved gaps by text and **passes on the rebased tree with no manual re-heal**, while `cargo xtask check` re-anchors the (now advisory) line numbers if run. Only a *new* uncovered text with no counterpart (a genuine lowering) fails the gate. Soundness is preserved by confirming a gap's own mapped line first, so a same-text line that genuinely regresses elsewhere is still caught (the net-zero-swap). Residual ambiguity is bounded to the case of several identical-text lines in one file where a removal and an unrelated same-text regression coincide.
