...
use common::media::{MediaReference, MediaReferenceKind};
    ...
    for LiveMediaReferenceOwnershipResolver<T>
{
    async fn resolve(
        ...
        let operation = async {
            ...
            while let Some((references, result)) = probes.next().await {
                match reported_probe_result(result) {
                    Some(Ownership::Foreign) => {
                        for reference in references {
                            foreign.prove_foreign(reference);
                        }
                    }
                    Some(Ownership::Owned | Ownership::Unknown) | None => {}
                }
            }
        };
    ...
    async fn resolve_local(
        ...
        instance_id: &InstanceId,
        base_url: Option<&BaseUrl>,
        mut local: LocalMediaSink,
    ) -> ProvenLocalMediaRefs {
        let mut targets = BTreeMap::<String, (Url, Vec<MediaReference>)>::new();
        for reference in references {
            match reference.kind() {
                MediaReferenceKind::Local => local.prove_local(reference.media().clone()),
                MediaReferenceKind::Absolute | MediaReferenceKind::SchemeRelative => {
                    let Some(target) =
                        target_for_form(reference.kind(), reference.reference_form(), base_url)
                    else {
                        continue;
                    };
                    targets
        ...
        let operation = async {
            ...
            tokio::pin!(probes);
            while let Some((references, result)) = probes.next().await {
                if matches!(reported_probe_result(result), Some(Ownership::Owned)) {
                    for reference in references {
...
/// Reports failed probes while preserving fail-closed handling for every
/// non-owned result. Both deletion and pre-write resolution share this boundary
/// so transport failures cannot silently turn into ordinary unknown evidence.
fn reported_probe_result(result: Result<Ownership, HeadTransportError>) -> Option<Ownership> {
    match result {
        Ok(ownership) => Some(ownership),
        Err(error) => {
            report_probe_error(&error);
            None
        }
    }
}
