...
impl SeededSession {
    ...
    /// Callers provide the exact post store and write scope.
...
pub async fn create_session_for(
    ...
    let username = users
        ...
        .expect("user exists")
        .username;
    let token = issue_session(sessions, write_scope, user_id).await;
    SeededSession {
        user_id,
        username,
        token,
...
async fn seed_and_session(
    ...
    let user = seed.seed(Arc::clone(&users), write_scope.clone()).await;
    let token = issue_session(sessions, write_scope, user.user_id).await;
    SeededSession {
...
pub async fn create_user_and_session(
    ...
    seed_and_session(
        ...
        sessions,
        write_scope,
        storage::test_support::SeedUser::new(),
    )
    .await
}
...
pub async fn create_operator_and_session(
    ...
    seed_and_session(
        ...
        sessions,
        write_scope,
        storage::test_support::SeedUser::new().operator(),
    )
    .await
}
