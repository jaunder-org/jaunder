...
async fn list_drafts_by_user_returns_only_drafts(#[case] backend: Backend) {
    ...
    let drafts = state
        .posts
        .list_drafts_by_user(user_id, None, parse_row_limit("10"), Utc::now())
        .await
        .unwrap();
...
async fn list_drafts_cursor_boundary(#[case] backend: Backend) {
    ...
    let all = state
        .posts
        .list_drafts_by_user(user, None, parse_row_limit("10"), Utc::now())
        .await
    ...
    let first = state
        .posts
        .list_drafts_by_user(user, None, parse_row_limit("1"), Utc::now())
        .await
        .expect("list_drafts_by_user failed");
...
async fn anonymous_is_not_admitted_by_an_empty_subscriber_ref(#[case] backend: Backend) {
    ...
    let listed: std::collections::HashSet<PostId> = state
        .posts
        .list_published(None, parse_row_limit("100"), &anon, Utc::now())
        .await
        .unwrap()
        .into_iter()
        .map(|p| p.post_id)
