...
7. Real e2e: `mail.ts` `extractLink`; `password_reset.spec.ts` + `email.spec.ts`
   assert the link is absolute, then follow its path (`new URL(link)`
   origin-strip + `goto`). `atompub.spec`/`helpers.ts` unchanged.
