...
Chosen option: Deploy as a **Single Binary** with an **Externalized Reverse Proxy**.

### Implementation Details

*   The application, storage (SQLite by default), and assets are bundled into a single binary.
*   Jaunder does not implement HTTPS directly; it expects to run behind a reverse proxy (e.g., Nginx, Caddy) that handles TLS termination.
