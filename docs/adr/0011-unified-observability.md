...
often, and is the rate abnormal". As of 2026-06-18 this addendum added an
...
- The OTLP `MeterProvider` was installed in `server::observability` next to the
  ...
  endpoint fed both traces and metrics; setup failure was non-fatal.
- When no endpoint was set (or in any non-server process — wasm, the CLI), no
  provider was installed and every instrument was a no-op, exactly like traces.

### Emit facade: `host::metrics` (originally `common::metrics`)

> **Superseded (2026-07-09, #345):** as of 2026-07-09 the facade moved to
> **`host::metrics`**, declared unconditionally. `host` is native-only
> (ADR-0058), so `opentelemetry` was kept out of the wasm bundle by crate
...
The PII rules above applied unchanged: metric attributes were bounded enums or
fixed operation names, never user input, so metrics carried no PII or secrets.

### Refinements over the spec catalog (as of 2026-06-18)
...
- `jaunder.posts{event}` counted **web-UI** post lifecycle actions; posts
  created or mutated over `AtomPub` were reflected in `jaunder.atompub.requests`
  instead, so the two never double-counted.
- `jaunder.atompub.requests{op,result}` was emitted from a single router-level
  middleware: `op` came from the matched route template + method (a bounded
  set), `result` from the response status class.
- Email metrics were emitted at the `web` send call sites (where the message
  `kind` is known), not inside the generic mailer.

### Testing (as of 2026-06-18)

Facade and mapping helpers were unit-tested against an in-memory metric reader
...
outcome, upload/serve outcome, AtomPub op/result) were additionally covered by
exhaustive table tests so every attribute mapping was exercised regardless of
...
installed for one-shot commands — `run()` called `init_tracing` for every
non-`serve` command — but both exporters were deferred: the metric reader was
periodic and the span processor batched, so they only exported on an interval
the long-running server easily reaches and a one-shot CLI command exits long
before. The CLI's metric and span emits were therefore installed correctly yet
silently dropped. The fix was to flush before exit, not to install a provider.

Convention adopted 2026-06-27: `init_tracing` returned a `#[must_use]`
`TelemetryGuard` owning the installed providers. A process held the guard for
its working scope; the guard's `Drop` called `shutdown()` (force-flush +
shutdown) on each provider, exporting buffered telemetry on every exit path —
success, `?` error-return, and panic unwind. A single binding at the `run()`
dispatch boundary owned telemetry for _every_ command — `serve` included — so
command bodies (including `cmd_serve`) carried no telemetry-lifecycle code; for
`serve` the guard was simply held for the process lifetime and flushed at
shutdown. Export failures (e.g. an unreachable collector) were logged, never
propagated — a telemetry failure must not change a command's exit status. This
closed the "CLI export" item the metrics addendum deferred (metrics **and**
traces, since both shared the drop-on-exit defect).
...
ADR-0058 had by then introduced `host` — the native-only sibling of `common` —
and the emitter set had grown to include `host` and `storage`. Every crate that
emitted metrics depended on `host` (`storage → host`, `web → host` under its
`server` feature, `server → host`, and `host` itself), and `host` was never in
the wasm dependency closure. As of 2026-07-09 the facade therefore moved to
**`host::metrics`**, declared **unconditionally**: `opentelemetry` was excluded
from wasm by crate structure, so the `metrics` feature on `common` — and the
`common/metrics` / `features = ["metrics"]` opt-ins in `host`, `server`, and
`web` — were deleted. This also removed `storage`'s prior reliance on Cargo
feature unification to see the metrics `SessionOutcome` enum (it thereafter
referenced `host::metrics` on a direct dependency).

No behavior change: the instrument catalog, bounded-enum cardinality discipline,
PII rules, and no-op-without-a-provider semantics were unchanged; only the
facade's crate home moved. Exporter setup remained in `server::observability`.
...
on naming. An unenforced convention is what allowed that, so as of 2026-07-29
the convention became a gate: `server-fn-tracing`, in `cargo xtask check` and
`cargo xtask validate`.

> **Partly superseded (2026-07-30, #714/#722):** as of 2026-07-30 the span-name
> half of this addendum no longer described the code. `server-fn-tracing`
> survived, but only as the recordable-type allowlist and its sibling
> default-denies — it neither wrote nor checked the span name, and it had no fix
> mode at all. The name was derived by `#[macros::server]` instead. The
> **values** and the **rule** were unchanged; the text below is the original,
> correct-for-its-time rationale for who maintained them. See the 2026-07-30
> addendum.
>
> **Trace-admission mechanism superseded (2026-08-25, #1138):** as of
...
> decision is `docs/adr/0156-sink-specific-telemetry-interfaces.md`.

### The span (as of 2026-07-29)

Every `#[server]` fn in `web/src` carried `#[tracing::instrument]`, placed
**after** `#[server]` (the arrangement the pre-existing sites used and that was
...
The name was a pure function of source location and identifier, so **the gate
wrote it**: `cargo xtask check` filled the `name = "…"` in (the same fix-mode
contract `fmt` has) and `cargo xtask validate` verified it by equality without
mutating. An author wrote `#[tracing::instrument(skip_all)]` and the derived
name landed in the source, so nothing was left to judgment and an operator
reading a span name could still grep for the literal.
`web/src/posts/api/listing.rs` yielded `posts`, not `api`. A `#[server]` fn
directly under `web/src` had no vertical directory and was a hard error rather
than a guessed name.

Only the name was written. A missing `#[tracing::instrument]` stayed a
_reported_ failure: inserting one would mean guessing the `skip(...)` list,
which is the one judgment the gate refused to make on an author's behalf.

Deriving the name rather than writing it had a payoff: the fn idents carried a
...
Spans used `#[tracing::instrument]`'s default **INFO** level, and no site set an
explicit `level`. Stated because it meant operator configuration reached trace
backends at INFO — permitted, since that data is the operator's own, but a
deliberate choice rather than an inherited default. This one was a **convention,
not an invariant**: the gate tolerated an explicit `level`, since level changes
verbosity rather than what is recorded.

### What may be recorded (as of 2026-07-29)

Every argument had to be either named in `skip(...)`/`skip_all` or have a type
on an explicit **recordable** allowlist. The list was **default-deny**: an
unlisted type was not recordable, so a newly-introduced argument type failed the
gate until someone classified it. The PII decision is forced when it arises
rather than left to a reviewer noticing.

The criterion is **"is this value already visible to the trace's reader, or
bounded by its own type?"** — not "did a user author it". Four grounds admitted
a type:
...
Everything else was skipped: secrets, `Email`, `Bio`, `DisplayName`,
...
This ADR states the rule; **the gate held the list** (`RECORDABLE_TYPES` in
...
registrar guard, the requirement was mandatory with **no per-fn opt-out**.

`fields(...)` value expressions were held to the same allowlist, since
...
`=`) was not checked — a field could be named after a skipped argument as long
as its value did not read it.

### Two caveats, recorded deliberately

- **`Bio`/`DisplayName` were skipped because, as of 2026-07-29, nothing
  published them.** They were reachable only through
  `profile::get`/`profile::update`. If a public `/@username` page later renders
  them, the classification warrants revisiting — the gate will not notice on its
  own.
- **Allowlisting `u32` left a narrow hole**: a numeric OTP or PIN would pass.
...
`#[instrument(err)]` and `ret` were **rejected** by the gate. Recording
server-fn failures as span errors is desirable but changes _what_ is recorded,
and needs its own PII review of the `WebError` `Display` chain — a different
question from span presence. Follow-ups filed as **#684** (path-based
`#[server]` matching, which unblocks dropping the vestigial fn-ident nouns) and
**#685** (`login`'s un-newtyped `label`).
...
identifier, then had **the gate write it into the source**. As of 2026-07-30,
#714 moved the derivation into `#[macros::server]` (`macros/src/server_fn.rs`),
which emitted `#[::tracing::instrument(name = "web.<vertical>.<ident>")]` — the
same value, from the same inputs, by the same rule. This resolved
...
### Why derivation was rejected in #511, and why it became available
...
`#[macros::server]` broke the coupling from the other side — it computed a
readable name from the file path and ident and wrote it into its own expansion —
so the span name no longer depended on what `#[server]` called its generated fn.

Superseded from the #511 addendum, item by item:

- **Nothing wrote the name into the source, and nothing checked it there.**
  `Mode::Fix` for span names was gone; `cargo xtask check` no longer rewrote
...
- **Presence was no longer a _reported_ failure — it was not representable.** A
  `#[macros::server]` fn always carried the instrument attribute, so the "a
  missing `#[tracing::instrument]` stays a reported failure" rule had nothing
  left to report.
- **`fields(…)`, `level`, `err`, and `ret` were rejected by the macro**, not
  ...
  `fields(who = %email)` hole; the macro closed it by construction instead, so
  the value-expression allowlist was deleted with it. The explicit-`level`
  tolerance went the same way — no site set one. Re-admitting `fields` means
  re-admitting the value allowlist in the same change.
- **The placement rule narrowed the derivation's input.** #511 noted
  `web/src/posts/api/listing.rs` yields `posts`, not `api`. #714 forbade that
...
What `server-fn-tracing` retained is the substance: the `RECORDABLE_TYPES`
default-deny over every parameter, the nameless-parameter rule, and default-deny
on any argument it does not model. It thereafter read `skip(…)`/`skip_all` from
`#[macros::server(…)]`, since no `#[tracing::instrument]` survived in source.
The macro's own key list was a second, independent default-deny, so adding a key
...
function, taken from the `boundary!` label. As of 2026-07-30 it was deleted
along with the label (`boundary!` itself went; `#[macros::server]` wrapped every
body in `crate::error::server_boundary` unconditionally). The field was
redundant with span context and strictly less precise: the failure event is
raised inside `web.<vertical>.<ident>`, whereas the bare label was the fn ident
alone — which, after #684 shed the vertical nouns, is ambiguous across verticals
(three `create`s).
...
So no operator lost the function's identity on any sink. The other five fields
