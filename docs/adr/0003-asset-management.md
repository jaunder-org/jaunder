...
To fulfill the goal of being a "single-binary" application, Jaunder needs a way
to serve static assets (CSS, JS, themes) without requiring external files to be
...
Chosen option: Use `rust-embed` to embed CSS and other static assets directly
into the server binary.

### Implementation Details

- `jaunder.css` and `jaunder-themes.css` are embedded using `rust-embed`.
- Dedicated Axum handlers serve these files from the binary, including ETag and
  conditional-request support (via `axum-embed`).
...
- Bad: Assets are coupled to the Rust build cycle (though `rust-embed` supports
  disk-reads in debug mode).
