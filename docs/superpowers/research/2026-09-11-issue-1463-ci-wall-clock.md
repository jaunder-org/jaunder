...
| Avoid duplicate coverage census binary build | The controlled instrumented run shows `cargo nextest list` before the separately instrumented `cargo llvm-cov nextest`; the 292,235-ms census stage may include a non-instrumented build. Treatment experiment 1 reached a 4,692-test census but failed before execution.                                     | **Hypothesis only; not selected.** `--no-report --no-clean` is invalid; the corrected `show-env` plus plain-nextest treatment still needs an isolated timing and union-verdict equivalence result.                                                                              |
