...
it lands in **C3** (media upload harvests `<content src>`/`content-type` from the
response — see ADR-0044), not C4; C4 then consumes the slug/published subset and
Unit D's `jaunder--atom->org` builds the full org-buffer synthesis on top of the
same primitive. (Option B of the C/D-boundary decision.)
