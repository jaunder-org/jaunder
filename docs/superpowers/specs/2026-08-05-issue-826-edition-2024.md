...
entirely test-side (79 `unsafe`-env call sites) plus five wasm-only view helpers
in `web` that are already proven fixable with a four-token change. Production
code is still _edited_ — three nested `if let` ladders collapse into let-chains
— but only where clippy asks and only in ways the gate verifies.
...
- Pin `resolver` explicitly in all three workspace roots (see D5).
...
- Clear the migration warnings (three `collapsible_if`, two `dead_code`).
...
- **Adopting resolver 3.** See D5; filed as a follow-up.
...
- `collapsible_if` × 3 — `storage/src` (lib), `web/src/tags/input_state.rs`,
  `client/src/dom.rs`. Clippy's suggested fix is a **let-chain**, 2024-only. Two
  of these are production code.
...
### D5 — Pin `resolver` explicitly; do not adopt resolver 3 here

There are three workspace roots. `Cargo.toml` and `tools/Cargo.toml` are virtual
and pin `resolver = "2"` explicitly, so they are unaffected. `xtask/Cargo.toml`
is a **package** workspace root, so flipping its package to edition 2024 would
_silently_ infer **resolver 3** — a feature-unification change with no visible
diff line.

A silent resolver change inside a migration whose premise is behaviour
preservation is unacceptable. `xtask/Cargo.toml` therefore gains an explicit
`resolver = "2"` under its `[workspace]` table, making all three roots
consistent and the current behaviour explicit. Adopting resolver 3 is real work
with real feature-unification consequences and is **filed as a separate issue**,
...
11. **AC11.** `xtask/Cargo.toml` declares `resolver = "2"` under `[workspace]`,
    and a follow-up issue for resolver-3 adoption exists and is linked here.
