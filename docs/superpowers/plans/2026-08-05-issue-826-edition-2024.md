...
**In:** 13 manifest edition flips · `resolver` pinned in `xtask` ·
doctest-harness fixture edition · `common::test_support::with_env` + conversion
of all env _access_ sites (mutating and reader-only) · `+ use<>` on 4 `web`
helpers · 3 let-chains · 2 mis-gated test structs · one ADR, promoted.

**Out:** #301's three contingent suppressions · resolver-3 adoption (T1) · any
behaviour change.
...
| #   | Task                                                                 | Gate                   |
| --- | -------------------------------------------------------------------- | ---------------------- |
| T1  | File the resolver-3 follow-up issue                                  | —                      |
...
| T8  | Pre-flip housekeeping: `resolver`, `dead_code` gating, stale comment | `cargo xtask check`    |
...
- [x] File via **`jaunder-issues`**, type Task, label `tooling`.

**Title:** `workspace: adopt resolver 3 now that all crates are edition 2024`

**Body:** all three workspace roots pin `resolver = "2"` explicitly as of #826;
edition 2024 would otherwise have inferred resolver 3 for `xtask` silently.
Resolver 3 changes feature unification across dev/build/target edges and needs
its own measured migration. Link #826.
...
- [ ] `cargo nextest run -p common test_support::env` — FAIL before, PASS after.
...
- [ ] Wrap **every** test that takes the lock — mutating or not — in `with_env`.
- [ ] `file_is_none_when_unset_or_blank` (117–126) is the interleaved case:
      remove → assert → set `"   "` → assert → remove. Express as **one**
      `with_env` with two `env.set`/`env.remove` steps and the assertions
      between them. Do **not** split it into two `with_env` calls — that reopens
      the race the single guard closes.
- [ ] `set_var(DIR_ENV, &d)` is path-valued — relies on `AsRef<OsStr>`.
- [ ] Delete `ENV_LOCK` and its stale comment.
- [ ] `cargo nextest run -p host capture` → PASS, same test names.
- [ ] Gate: `cargo xtask check` → PASS.
- [ ] Commit: `refactor(host): route capture env tests through with_env (#826)`
...
- [ ] Wrap every lock-taking test, mutating or not.
- [ ] Check `postgres/mod.rs:328-338` for the interleaved shape (sets
      `JAUNDER_DB_PASSWORD`, asserts, then `_FILE`, asserts). If the assertions
      interleave, keep it one `with_env`.
- [ ] `postgres/mod.rs:333` is path-valued.
- [ ] Delete both `ENV_LOCK`s.
- [ ] Existing tests only — do not convert a `#[tokio::test]` to `#[test]` or
      vice versa, and the dual-backend template does not newly apply.
- [ ] `cargo nextest run -p storage` → PASS.
- [ ] Gate: `cargo xtask check` → PASS.
- [ ] Commit: `refactor(storage): route env tests through with_env (#826)`
...
- [ ] Convert all 32. Mutators get `env.set`/`env.remove`; readers become
      `with_env(|_env| { … })`.
- [ ] The delta must wrap the `parse(...)` call, not just the assertion — clap
      reads env at parse time.
- [ ] Delete `ENV_LOCK` at 433; its 430–432 comment is superseded by
      `with_env`'s docs.
- [ ] Confirm the converted count: `rg -c 'with_env' server/src/cli.rs` → 32.
- [ ] `cargo nextest run -p jaunder cli` → PASS. (Crate is `jaunder`, not
      `server`.)
- [ ] Gate: `cargo xtask check` → PASS.
- [ ] Commit: `refactor(server): route cli env tests through with_env (#826)`
...
- [ ] `xtask/Cargo.toml` — add `resolver = "2"` under `[workspace]` (AC11). A
      no-op today; the whole point of spec D5 is that it is explicit _before_
      the flip would otherwise infer resolver 3 silently. Verify `Cargo.toml:2`
      and `tools/Cargo.toml:2` still read `resolver = "2"`.
- [ ] `web/src/error.rs` — add `#[cfg(feature = "server")]` to the `SourceError`
      (140) and `OuterError` (151) definitions. They are **not** dead: they are
      used at 218, 219, 221, 250, 267, 305, 378, 438, 472, 529 in tests already
      gated on that feature. Do **not** delete them and do **not**
      `#[allow(dead_code)]`.
- [ ] `test-support/tests/cli.rs:12` — reword the stale doc comment so it no
      longer contains the literal `set_var` (AC4). Its claim is about process
      isolation and can be said without the identifier.
- [ ] Gate: `cargo xtask check` → PASS.
- [ ] Commit:
      `chore: pin xtask resolver, gate test-only structs, refresh a stale comment (#826)`
...
- [ ] AC11 `xtask/Cargo.toml` has `resolver = "2"`; T1's issue number recorded
