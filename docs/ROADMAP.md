...
Project foundation established with clean workspace organization and CI enforcement.

### M1: Deployment bootstrap
`jaunder init` and guided setup implemented.

### M2: User management
Authentication, registration, and profile management (display name, bio, avatar) implemented.
...
CLI-based `backup` and `restore` commands implemented for both database and media.
