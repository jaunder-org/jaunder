...
//!
//! The pure codec lives in `common::session_user` (so `test-support` can build
//! markers without linking `web`, #791) and is re-exported here; the wasm-only
//! `localStorage` binding lives in
//! [`super::marker_storage`] (#514).

pub use common::session_user::{SessionUser, decode_marker, encode_marker};
