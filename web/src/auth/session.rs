...
/// into the seed signal AND the marker (so the next boot stays flash-free).
...
/// Optimistically set the session (login/register) — seed signal + marker, so the
/// chrome flips without waiting for the reconcile round-trip.
pub fn set_session(user: SessionUser) {
    let ctx = use_session();
    marker_storage::set(&user);
    ctx.seed.set(Some(user));
}
