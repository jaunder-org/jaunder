//! Server-only half of `web::error`: the `InternalError` carrier re-export, the
//! `kind → WebError` projection, and the `#[server]` boundary wrapper.
//!
//! Gated once on the `mod server;` declaration in [`super`] (ADR-0070), so nothing
//! in here carries a per-item `server` feature gate.
...
pub(super) fn emit_arg_decode_failure(value: &ServerFnErrorErr) {
    if !matches!(
        value,
...
mod tests {
    use super::{
        ErrorClass, ErrorKind, InternalError, WebError, WebResult, project, server_boundary,
    };
